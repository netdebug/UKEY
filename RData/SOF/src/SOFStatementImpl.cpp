...
SOFStatementImpl::SOFStatementImpl(Poco::Data::SessionImpl& rSession, sqlite3* pDB):
...
void SOFStatementImpl::compileImpl()
{
	if (!_pLeftover)
	{
...
void SOFStatementImpl::bindImpl()
{
	_stepCalled = false;
	_nextResponse = 0;
	if (_pStmt == 0) return;
...
std::size_t SOFStatementImpl::next()
{
...
std::size_t SOFStatementImpl::columnsReturned() const
{
	return (std::size_t) _columns[currentDataSet()].size();
}


const MetaColumn& SOFStatementImpl::metaColumn(std::size_t pos) const
{
	std::size_t curDataSet = currentDataSet();
...
int SOFStatementImpl::affectedRowCount() const
{
